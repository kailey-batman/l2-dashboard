    ...
    if results_df is not None and not results_df.empty:
        ...
        with col1:
            with st.container():
                ...
                if st.button(f"Total Escalations\n\n**{total}**\n\n↓ Drill Down", key="btn_total", use_container_width=True):
                    st.session_state.metric_filter = None
                    st.rerun()
        ...
        with col2:
            ...
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"L2 Can Support\n\n**{supported}** ({pct})\n\n↓ Drill Down", key="btn_supported", use_container_width=True):
        ...
        with col3:
            ...
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"L2 Cannot Support\n\n**{unsupported}** ({pct})\n\n↓ Drill Down", key="btn_unsupported", use_container_width=True):
                st.session_state.metric_filter = ("decision", "L2 Cannot Support")
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with col4:
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"Partially Supported\n\n**{partial}**\n\n↓ Drill Down", key="btn_partial", use_container_width=True):
                st.session_state.metric_filter = ("decision", "Partially Supported")
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with col5:
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"Insufficient Data\n\n**{insufficient}**\n\n↓ Drill Down", key="btn_insufficient", use_container_width=True):
        ...
        with col6:
            ...
            <div style="background-color: #373E47; border: 1px solid #444C56; border-radius: 10px;
                        padding: 16px 12px; min-height: 110px;">
                <div style="color: #9E9E9E; font-size: 0.85rem;">Avg Confidence</div>
                <div style="color: #E0E0E0; font-size: 1.8rem; font-weight: 700;">{avg_conf:.1f}/5</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with l2_col1:
            ...
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"L2 Involved\n\n**{len(l2_involved)}** ({pct})\n\n↓ Drill Down", key="btn_l2_involved", use_container_width=True):
                st.session_state.metric_filter = ("l2_involvement", "!=None")
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with l2_col2:
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"L2 Responsible\n\n**{len(l2_responsible)}**\n\n↓ Drill Down", key="btn_l2_responsible", use_container_width=True):
                st.session_state.metric_filter = ("l2_involvement", "Responsible")
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with l2_col3:
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"L2 Assisted\n\n**{len(l2_assisted)}**\n\n↓ Drill Down", key="btn_l2_assisted", use_container_width=True):
                st.session_state.metric_filter = ("l2_involvement", "Assisted")
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with l2_col4:
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"Sean\n\n**{len(sean_tickets)}**\n\n↓ Drill Down", key="btn_sean", use_container_width=True):
                st.session_state.metric_filter = ("l2_engineer", "Sean")
                st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        with l2_col5:
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"Jayson\n\n**{len(jayson_tickets)}**\n\n↓ Drill Down", key="btn_jayson", use_container_width=True):
        ...
        if len(could_but_didnt_df) > 0:
            st.markdown('<div class="metric-btn">', unsafe_allow_html=True)
            if st.button(f"Gap: {len(could_but_didnt_df)} tickets L2 could have supported but had no L2 involvement  ↓ Drill Down", key="btn_gap", use_container_width=True):
